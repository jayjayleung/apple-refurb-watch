...
这是一次打包与分发改进版本，在 0.3.1 的基础上同时提供目录版和单文件可执行版。

## 主要变化

- **双格式发布**：Windows、macOS Apple Silicon 和 Linux / NAS 均提供目录版 zip 与单文件可执行文件。
- **单文件后台运行**：`serve --detach` 会为后台进程重新解压运行时文件，避免单文件父进程退出后临时目录被清理。
- **构建烟测**：发布构建会验证版本命令、桌面探测和后台服务健康检查。

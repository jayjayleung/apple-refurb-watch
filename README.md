...
4. 开机自启：设置「这台电脑」→「开机自启」，或 `apple-refurb-watch service install`（安装包默认 `--tray`）。登录后拉起托盘，可 `service start` / `stop` / `restart`。本机桌面模式会隐藏「服务」里的开机自启，避免再装一份 `serve`。
...
1. 先按下面「只要网页」把**服务**跑起来，打开「允许远程访问」，记下口令。服务开机用设置「服务」→「开机自启」，或 `service install --serve`。
...
本机不再扫描、不写第二份库。托盘退出只关窗口，不动服务器。连上之后，设置「服务」里的开机自启改的是 **NAS 那台机器**；「这台电脑」里的开机自启仍是这台电脑的托盘。
...
也可在设置页勾选「服务」→「开机自启」。Linux 走 systemd --user，macOS 走 LaunchAgent，Windows 走登录计划任务。同一系统只装一份任务：要么托盘，要么服务。
...
3. 到「设置」打开通知通道（Bark / Server酱 / PushPlus / 飞书 / 钉钉 / Telegram / 邮件），点「发送测试」。密钥只在这一页填
...
7. 扫描所在的那台机器要开机自跑：设置「服务」→「开机自启」。桌面包本机请用「这台电脑」的开机自启（本机模式会隐藏服务开关）

...
def _settings_input_tag(html: str, name: str) -> str:
    marker = f'name="{name}"'
    pos = html.find(marker)
    assert pos != -1, f"missing input {name}"
    start = html.rfind("<input", 0, pos)
    end = html.find(">", pos)
    assert start != -1 and end != -1
    return html[start : end + 1]


def test_settings_page_masks_saved_secret_placeholders(tmp_path) -> None:
    from apple_refurb_watch.settings import NOTIFY_CHANNEL_UI

    secrets = {
        "bark-url": "https://api.day.app/saved-bark-key-xyz",
        "serverchan-sendkey": "SCT12345savedSendKey",
        "pushplus-token": "pushplus-saved-token-xyz",
        "feishu-webhook": "https://open.feishu.cn/open-apis/bot/v2/hook/saved-feishu",
        "dingtalk-webhook": "https://oapi.dingtalk.com/robot/send?access_token=saved-ding",
        "dingtalk-secret": "SECdingtalkSavedSecret",
        "telegram-bot": "123456:TELEGRAM-SAVED-BOT",
